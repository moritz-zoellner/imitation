...
def main():
    ...
    if env is None and dataset is not None:
        env = dataset.get_env()
    ...
    rollouts: dict[str, jnp.ndarray] = env.rollout(policy, n_episodes=100, episode_length=1000, seed=0)

    average_return = rollouts["reward"].sum(axis=1).mean()
